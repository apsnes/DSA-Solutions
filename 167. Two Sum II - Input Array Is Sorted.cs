...
public class Solution
{
    public int[] TwoSum(int[] numbers, int target)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            var required = target - numbers[i];
            var left = i + 1;
            var right = n - 1;
            while (left <= right)
            {
                var mid = (left + right) / 2;
                if (numbers[mid] == required) return new int[2] { i + 1, mid + 1 };
                if (numbers[mid] > required) right = mid - 1;
                else if (numbers[mid] < required) left = mid + 1;
            }
        }
        return new int[2];
    }
